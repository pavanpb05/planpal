...
export default function Page() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setSuccess(
        "Registration successful! Verification email sent. Redirecting to login..."
      );
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center px-4">
      <main className="w-full max-w-lg rounded-3xl border border-slate-800 bg-slate-900/80 shadow-[0_18px_60px_rgba(15,23,42,0.9)] px-6 py-8 space-y-6 text-slate-100">
        <header className="space-y-1">
          ...
          <p className="text-sm text-slate-300">
            Tell us a bit about you so we can make your group travel smoother.
          </p>
        </header>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="Beach trips, treks, food trails..."
              required
              className="min-h-20 w-full rounded-xl border border-slate-700 bg-slate-900/60 px-3 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-y"
            />
          </div>
          ...
          <label className="flex items-start gap-2 text-xs text-slate-300">
            <input
              ...
              className="mt-0.5 h-4 w-4 rounded border-slate-600 bg-slate-900/80"
            />
            <span>
              I agree to the{" "}
              <span className="text-indigo-300">Terms and Conditions</span>.
            </span>
          </label>
          ...
          <p className="text-sm text-rose-400 bg-rose-500/10 border border-rose-500/40 rounded-xl px-3 py-2">
            ...
            {emailExists && (
              <span>
                ...
                <a
                  href="/login"
                  className="text-indigo-300 underline-offset-2 hover:text-emerald-200"
                >
                  Login here
                </a>
              </span>
